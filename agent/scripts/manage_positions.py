...
def main() -> int:
    ...
    if ks == "FLATTEN_ALL":
        log.info("kill switch FLATTEN_ALL — position management skipped (the flatten path owns the book)")
        return 0
        ...
        with repo.session() as session:
            for p in positions_raw:
                ...
                if cov is None or cov.indeterminate_qty > 0:
                    ...
                    # neither protection nor its absence. Amending on that guess
                    # is how a lot ends up with two stops, which is a short
                    # waiting for a gap down.
        ...
        for act in actions:
            if isinstance(act, RatchetStop):
                ...
                log.info(
                    ...
                    act.ticker, act.quantity, act.bars_held, act.pnl_pct * 100,
                    "" if args.submit else "   [dry run]",
                )
